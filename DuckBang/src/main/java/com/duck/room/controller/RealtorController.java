...
public class RealtorController {
	...
	public String regist(HttpServletRequest request, HttpServletResponse response, Model model) throws NumberFormatException, ParseException, IOException {
		...
		if(request.getParameter("realtor_agreeChk") == null) {
			...
			rm.addLocation(new Location(l_code, request.getParameter("l_xcordinate"), request.getParameter("l_ycordinate")));
			rm.addOffering(new Offering(o_id, "pass", new Date(new java.util.Date().getTime()), "5", l_code));
			rm.addKind(new OfferingKind(o_id, request.getParameter("ok_code"),
			...
			rm.addDetail(new OfferingDetail(
					o_id, 
			...
			rm.addOption(new OfferingOption(
					o_id, 
					optionChk[0], 
					optionChk[1], 
					optionChk[2], 
			...
			rm.addSecurity(new OfferingSecurity(
					o_id,
					securityChk[0], 
					securityChk[1], 
					securityChk[2], 
